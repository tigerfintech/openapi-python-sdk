...
class Strategy(object):
    def __init__(self, push_client=None, trade_client=None, quote_client=None, context=None):
        ...
        if settings.EVENT_TRIGGER:
            ...
            self.lunch_break = datetime.strptime(settings.LUNCH_BREAK, '%H%M%S').time()
            self.afternoon_start = datetime.strptime(settings.AFTERNOON_START, '%H%M%S').time()
    ...
    def on_ticker(self, symbol, items, hour_trading):
        ...
        if symbol in self.symbol_set:
            if self.tick_util.on_latest_price(symbol, latest_price):
                ...
                self.trade_client.create_order(self.context.account, contract, 'BUY', 'MKT', symbol_market['per_trade'],

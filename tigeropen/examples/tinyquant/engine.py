...
from tigeropen.examples.tinyquant.client import client_config, push_client, trade_client, quote_client, \
    global_context
import tigeropen.examples.tinyquant.settings as settings
...
timezone = pytz.timezone(settings.MARKET.TIMEZONE)
IS_EVENT_TRIGGER = settings.EVENT_TRIGGER
...
def on_quote_changed_event_trigger(symbol, items, hour_trading):
    ...
    if symbol in subscribe_symbols:
        if settings.MARKET.name != 'HK' and settings.MARKET.name != 'US':
...
if __name__ == '__main__':
    ...
    try:
        if IS_EVENT_TRIGGER:
            OPEN_TIME = settings.MARKET.OPEN_TIME
            CLOSE_TIME = settings.MARKET.CLOSE_TIME
            SYSTEM_DELAY = settings.SYSTEM_DELAY
            ...
            if settings.FREQUENCY == 'minute':
                ...
                if settings.MARKET.LUNCH_BREAK_START_TIME:
                    lunch_break_start = datetime.strptime(settings.MARKET.LUNCH_BREAK_START_TIME, '%H:%M:%S').time()
                    lunch_break_end = datetime.strptime(settings.MARKET.LUNCH_BREAK_END_TIME, '%H:%M:%S').time()
                    while True:

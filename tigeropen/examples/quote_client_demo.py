    ...
    while not is_last_page:
        ...
        if result.total_page:
            for item in result.items:
                ...
                print(
                    f'page:{result.start}, symbol:{symbol}, base_filter1 value:{base_filter1_value}, accumulate_filter value:{accumulate_filter_value}')
            print(f'current page symbols:{result.symbols}')
            scanner_result_symbols.update(result.symbols)
        time.sleep(1)
        # 处理分页
        if result.start >= result.total_page - 1:
            is_last_page = True
        else:
            begin_page = result.start + 1
